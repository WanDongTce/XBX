...
Page({
        ...
        network.POST({
            ...
            success: function (res) {
                ...
                if (res.data.code == 200) {
                    ...
                    D = that.change(date.getDate()) + ' ';
                    h = that.change(date.getHours()) + ':';
                    m = that.change(date.getMinutes());
                    s = that.change(date.getSeconds());

                    if (a.type == 1) {
                        that.setData({
                            sctdidx: 1,                           
                            navigationtext: '支出',
                            selectPayListId: a.financetypeid,//支出分类id
                            pay_money: a.price,//支出的钱
                            date1: Y + M + D,
                            time1: h + m,
                            textarea1: a.remark,
                            imgList:a.picture,
                            imgListNew: a.picture,
                        });
                        ...
                        that.setData({
                            sctdidx: 2,
                            navigationtext: '收入',
                            selectShouruListId: a.financetypeid,//支出分类id
                            shouru_money: a.price,//支出的钱
                            date2: Y + M + D,
                            time2: h + m,
                            textarea2: a.remark,
                        });
      ...
      var submitdata111 = ((Date.parse(new Date(submitdata1))) / 1000).toString();
        ...
        if (that.data.imgList.length == 0) {
            ...
            if (that.data.delImg){                              
                ...
                var obj={};
                obj[0] = that.data.imgList[0].url;
                // console.log(that.data.imgList[0].url)
                // console.log(JSON.stringify(obj))
                that.getSubmit(1, that.data.pay_money, that.data.selectPayListId, that.data.textarea1, submitdata111, obj);
            }
        }          
    }
  },
      ...
      var submitdata222 = ((Date.parse(new Date(submitdata2))) / 1000).toString();
        ...
        if (that.data.imgList.length == 0) {
            ...
            if (that.data.delImg) { 
                ...
                var obj = {};
                obj[0] = that.data.imgList[0].url;
                that.getSubmit(2, that.data.shouru_money, that.data.selectShouruListId, that.data.textarea2, submitdata222, obj);
            }
        }
  ...
  //提交方法
  getSubmit: function (newtype, newprice, financetypeid, remark, financedate,obj) {
    ...
    addImg: function () {
        ...
        wx.chooseImage({
            ...
            success: function (res) {
                ...
                // console.log(that.data.imgListNew[0].url)
            }
        });
    },
    delImg: function (e) {
        ...
        console.log(that.data.imgListNew.length)
    },
})